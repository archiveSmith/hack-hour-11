...
function getPINs(observed) {
  ...
  // if we only observe a single digit, return the combo
  if (observed.length === 1) return possible_nums[observed];

  const remaining_nums = getPINS(observed.slice(1));

  return possible_nums[observed[0]].reduce((acc, num) => [...acc, ...remaining_nums.map(pins => num.concat(pins))], []);
}
