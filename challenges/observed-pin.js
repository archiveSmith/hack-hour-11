...
function getPINs(observed) {
  ...
  function recursePin(pin) {
    if (pin.length === observed.length) masterArr.push(pin);
    else {
      for (let i = 0; i < observed.length; i++) {
        const objNumArr = masterPlan[observed[i]];
        for (let j = 0; j < objNumArr.length; j++) {
          const newPin = observed;
          newPin.splice(i, 1, objNumArr[j]);
          recursePin(newPin)
        }
      }
    }
